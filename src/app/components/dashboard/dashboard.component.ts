...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  rfidData: any = null;
  errorCardMessage: string | null = null; // Message d'erreur pour la carte invalide
    ...
    this.rfidService.getRFIDStatus().subscribe((data: any) => {
      ...
      if (data && data.user && data.user.nom) {
        ...
        if (this.selectedUser) {
          this.errorCardMessage = null;  // Réinitialise le message d'erreur
          this.showModal();
        } else {
          this.errorCardMessage = 'Utilisateur non trouvé pour cette carte.';
          this.showModal();
          console.error('Utilisateur non trouvé pour le nom:', data.user.nom);
        }
      } else {
        this.errorCardMessage = 'Carte invalide ou données incorrectes.';
        this.showModal();
