...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  listenForRFID() {
    this.rfidService.getRFIDStatus().subscribe((data: any) => {
      console.log('Données reçues de la carte RFID:', data);  // Affichez les données dans la console
      
      // Vérifiez que data.user et data.user.nom existent
      if (data && data.user && data.user.nom) {
        this.detectedCardInfo = data;  // Stockez les informations dans une variable
  
        // Recherchez l'utilisateur en fonction du nom
        this.selectedUser = this.employees.find(
          (employee) => employee.nom === data.user.nom
        );
  
        // Si un utilisateur est trouvé, affichez le modal
        if (this.selectedUser) {
          this.showModal();  // Affiche le modal avec les données de l'utilisateur
        } else {
  ...
  showModal() {
    ...
    // Assurez-vous que les informations de l'utilisateur sont présentes
    if (this.selectedUser) {
      // Définir les informations à afficher dans le modal
      const modalTitle = modalElement.querySelector('.modal-title');
      const modalBody = modalElement.querySelector('.modal-body');
      
      // Mettez à jour le contenu du modal
      if (modalTitle) {
