...
export class RFIDService {
  ...
  /**
   * Méthode pour récupérer l'UID de la carte RFID via API REST
   * @returns Observable qui émet l'UID de la carte
   */
  getCardUID(): Observable<any> {
    return this.http.get<any>(`${environment.apiUrl}/api/data`).pipe(
      catchError((error: HttpErrorResponse) => {
        console.error('Erreur lors de la récupération du UID de la carte RFID :', error);
        return throwError(() => new Error(error.message || 'Erreur inconnue lors de la récupération de l\'UID.'));
      })
    );
  }
