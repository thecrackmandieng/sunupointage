...
parser.on("data", (data) => {
  try {
    console.log("Données brutes reçues de l'Arduino :", data);

    if (data.includes("{") && data.includes("}")) {
      lastCardData = JSON.parse(data);
      console.log("Données analysées et stockées :", lastCardData);
    }
  } catch (error) {
...
app.get('/api/data', (req, res) => {
  if (lastCardData) {
    res.json(lastCardData); // Retourne les données si elles existent
  } else {
